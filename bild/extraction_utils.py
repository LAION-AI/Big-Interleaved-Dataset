...
def parser_bytes(url, html_byte):
    ...
    for ele in tree.body.get_elements_by_tag_name("img"):
        ...
        imgs[f"###img#{page_config['img_count']}###"] = bildtuple(
    ...
    for ele in tree.body.get_elements_by_tag_name("iframe"):
        ...
        iframedict[f"###iframe#{page_config['iframe_count']}###"] = bildtuple(
            chash, get_extension(csrc), csrc
        )
    ...
    for ele in tree.body.get_elements_by_tag_name("video"):
        ...
        if len(ele.get_elements_by_tag_name("source")) > 0:
            ...
            vids[f"###video#{page_config['vid_count']}###"] = bildtuple(
                chash, get_extension(csrc), csrc
            )
            nele = tree.create_element("img")
            nele["src"] = csrc
    ...
    for ele in tree.body.get_elements_by_tag_name("audio"):
        ...
        if len(ele.get_elements_by_tag_name("source")) > 0:
            ...
            auds[f"###audio#{page_config['aud_count']}###"] = bildtuple(
                chash, get_extension(csrc), csrc
            )
            nele = tree.create_element("img")
            nele.setattr("src", csrc)
    ...
    bildrecord = chunks2darray(
        chunks=fmttext,
        vids=vids,
        imgs=imgs,
        auds=auds,
        iframedict=iframedict,
        pg_stats=page_config,
    )

    assert len(bildrecord) == len(
        fmttext
    ), f"{len(bildrecord)},{len(fmttext) }Total Media and Text chunks don't match!"
