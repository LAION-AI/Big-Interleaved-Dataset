...
def pipeline2(warcf="/fsx/home-harrysaini/raw_warc/CC-MAIN-20220807150925-20220807180925-00000.warc.gz",wurl="/fsx/home-harrysaini/raw_warc/CC-MAIN-20220807150925-20220807180925-00000.warc.gz",config=config):
        ...
        # try:
            ...
            if record.headers["WARC-Type"]=="response" and record.content_length>=128:
                ...
                if content_type.startswith("text/html"):
                    ...
                    page_config,bildrecord,imgs,vids,auds,iframedict = parser_bytes(url,html_bytes)
    ...
    warc_htmldf.to_parquet(f"{config['Extraction_store']}/{wurl.split('.')[1].split('/')[-1][-5:]}.parquet")

    warc_imgdf = pd.DataFrame(warcimg, columns=["id","md5hash","file_extension","url"])
    warc_imgdf.to_parquet(f"{config['Imgstore']}/{wurl.split('.')[1].split('/')[-1][-5:]}imgs.parquet")

    warc_viddf = pd.DataFrame(warcvid, columns=["id","md5hash","file_extension","url"])
    warc_viddf.to_parquet(f"{config['Vidstore']}/{wurl.split('.')[1].split('/')[-1][-5:]}vids.parquet")

    warc_auddf = pd.DataFrame(warcaud, columns=["id","md5hash","file_extension","url"])
    warc_auddf.to_parquet(f"{config['Audstore']}/{wurl.split('.')[1].split('/')[-1][-5:]}auds.parquet")

    warc_iframedf = pd.DataFrame(warciframe, columns=["id","md5hash","file_extension","url"])
    warc_iframedf.to_parquet(f"{config['Iframestore']}/{wurl.split('.')[1].split('/')[-1][-5:]}iframes.parquet")
