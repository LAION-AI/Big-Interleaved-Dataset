...
def chunks2darray(
    chunks: List[str],
    vids: Dict[str, NamedTuple],
    auds: Dict[str, NamedTuple],
    imgs: Dict[str, NamedTuple],
    iframedict,
    pg_stats: Dict,
) -> List[Document]:
    ...
    for x in chunks:
        if x.startswith("video#", 3):
            ...
            output_list.append(
                Document(bildvideo(vid=None, bildtype="video", md5hash=vids[x].hashm))
            )
            ...
            output_list.append(
                Document(bildimage(img=None, bildtype="image", md5hash=imgs[x].hashm))
            )
            ...
            output_list.append(
                Document(bildaudio(aud=None, bildtype="audio", md5hash=auds[x].hashm))
            )
            ...
            output_list.append(
                Document(bildany(bildtype="iframe", md5hash=iframedict[x].hashm))
            )
        else:
